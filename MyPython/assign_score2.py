...
def setMinExp(nwk,exp,expCut):
    ...
    filtered_gene = exp[exp.max(axis=1)>1]['Gene_Symbol']
...
def expCut(nwk,exp,sample_list,expCut):
    ...
    gene_selected = exp[boolean_mask]['Gene_Symbol']
...
if __name__=="__main__":
    ...
    mask=np.logical_and(STRING['protein1'].isin(exp['Gene_Symbol']),STRING['protein2'].isin(exp['Gene_Symbol']))
    STRING=STRING[mask]
    ...
    STRING_FCcut_corrCut[['protein1','protein2','string*corr']].to_csv(args.cancerType+'.filtered.noExpCut.sorted',index=False,sep='\t')
    end=time.time()
    time_elapsed=end-start
    print >> sys.stderr, 'network_correaltion: '+str(time.strftime("%H:%M:%S",time.gmtime(time_elapsed)))
